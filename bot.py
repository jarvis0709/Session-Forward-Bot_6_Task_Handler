        ...
        for dest_channel in destination_channels:
            try:
                ...
                if event.media:
                    ...
                    task = asyncio.create_task(steallootdealUser     .send_message(dest_channel, message_text, file=event.media))
                    tasks.append(task)
                    logging.info(f"Forwarded media message to channel {dest_channel}")

                else:
                    task = asyncio.create_task(steallootdealUser     .send_message(dest_channel, message_text))
                    tasks.append(task)
